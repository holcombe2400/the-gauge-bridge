...
5. For a Starlink router, open the Starlink app: **Settings → Router → Advanced → Custom DNS**. Set the primary DNS server to the Home Assistant IP address. The Bridge handles the one Vevor address locally and safely forwards all other DNS lookups. For another router, either make the same custom-DNS change or add a host override for `rtupdate.wunderground.com`.
...
- `lastStationPacketAt` empty: the station has not reached the Bridge. Recheck the Starlink Custom DNS setting (or router DNS override) and the Vevor Station ID/key.
